...
const missingConnectionSupplier: ConnectionSupplier = () => {
  throw new Error("Sql does not have connection provided. Use ctx.sql to create initial Sql.")
}
...
export class Sql {
  ...
  constructor(
    parts: TemplateStringsArray | string[] | string,
    private params: any[],
    private connectionSupplier: ConnectionSupplier = missingConnectionSupplier
  ) {
  ...
  append(
    right?: string | Sql | any,
    connectionSupplier: ConnectionSupplier = this.connectionSupplier
  ): Sql {
    if (!right) {
      return this
    }
    ...
    if (right instanceof Sql) {
      ...
      const parts = [
        ...
        this.parts[this.parts.length - 1] + " " + concat,
        ...remaining,
      ]
  ...
  async aggregate<T extends {[x: string]: string}>(
    aggregations: T
  ): Promise<{[x in keyof T]: number}> {
    const exprs = Object.keys(aggregations).map(k => `${aggregations[k]} as ${k}`)

    return (await this.wrap(sql(`select ${exprs.join(",")} from (`), sql`) as t`).one({})) as any
  }
  ...
  async one(def: any = undefined): Promise<any> {
  ...
  private prepareQueryParams() {
    return this.params.map(p => (p === undefined ? null : p)).map(filterOutUndefinedProps)
  }
  ...
  private convertTypesOnReading(rows: any[], fields: any[]) {
  ...
  async page(
    request: PageRequest,
    totalAggregations: {count: string; [x: string]: string} = {count: "count(*)"}
  ): Promise<Page<any>> {
    ...
    const rowsQuery = this.append(sort && sql(`order by ${sort} ${order}`))
      .append(size && sql`limit ${size}`)
      .append(page && size && sql`offset ${page * size}`)
  ...
  async prevNextIds(id: number): Promise<Array<{id: number; rowNum: number}>> {
    ...
    const {rowNum} = await rowNums.append(`where id = ${id}`).one()
...
export function sql(parts: TemplateStringsArray | string[] | string, ...params: any[]) {
...
export function enableTrace(enabled: boolean) {
  trace = enabled
}

function filterOutUndefinedProps(o: any) {
  if (!o) return o
